...
public class OpenApiConfig {
    ...
    OpenAPI collabOpenApi() {
        return new OpenAPI().info(new Info()
                .title("Collab API")
                .version("1.0.0")
                .description("프로젝트 협업 도구 REST API"));
    }
    ...
    public OperationCustomizer currentUserHeaderCustomizer(
        ...
        return (operation, handlerMethod) -> {
            ...
            Set<String> currentUserParameterNames = IntStream.range(0, methodParameters.length)
                    .filter(index -> Arrays.stream(methodParameters[index].getParameterAnnotations())
